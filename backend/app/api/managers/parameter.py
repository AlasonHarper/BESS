...
async def delete_parameter(id,user_id):
    ...
    if len(params) == 0:
        paramId = 0
        await database.execute(user.update().where(user.c.id == user_id).values(parameter_id = 0))  
    else:
        currentUser = await database.fetch_one(query=user.select().where(user.c.id == user_id))
        paramId =currentUser.parameter_id
